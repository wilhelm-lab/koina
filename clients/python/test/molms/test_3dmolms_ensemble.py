...
def test_inference():
    structures = ["CCCCCCC"]
    bareserver = SERVER_HTTP[len("http://") :]
    ...
    precursor_type = httpclient.InferInput(
        "precursor_type", precursor_type_in.shape, "BYTES"
    )
    precursor_type.set_data_from_numpy(precursor_type_in)

    collision_energy = httpclient.InferInput(
        "collision_energy", collision_energy_in.shape, "FP32"
    )
    ...
    response = triton_client.infer(
        MODEL_NAME, inputs=[SMILES, precursor_type, collision_energy], outputs=[output]
    )
