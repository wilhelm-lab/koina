...
def test_inference():
    SEQUENCES = np.array(
        [
            ["[]-AA-[]"],
            ["[UNIMOD:9990]-PEPTIPEPTIPEPTIPEPTIPEPTIPEPT-[]"],
            ["[]-RHKDESTNQK[UNIMOD:9990]CGPAVILMFYW-[]"],
    ...
    ces = np.array([[32.0] for _ in range(len(SEQUENCES))], dtype=np.float32)
    ...
    assert np.allclose(
        intensities,
        np.load("test/Prosit/arr_Prosit_2025_intensity_ptm2_int.npy"),
        rtol=0,
        atol=1e-3,
        equal_nan=True,
    )
