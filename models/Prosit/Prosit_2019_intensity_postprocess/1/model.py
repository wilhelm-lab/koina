...
class TritonPythonModel:
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            precursor_charge_in = (
                pb_utils.get_input_tensor_by_name(request, "precursor_charge_in:0")
                .as_numpy()
            )
            peaks_in = (
                pb_utils.get_input_tensor_by_name(request, "peaks_in:0")
                .as_numpy()
            )
            ...
            output = pb_utils.Tensor("out/Reshape:1", peaks.astype(self.output_dtype))
