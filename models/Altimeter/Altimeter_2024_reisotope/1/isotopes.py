...
class CubicSpline:
    ...
    def eval(self, x):
        ...
        xx -= (index * 1000)
        return max(0,((self.d[index] * xx + self.c[index]) * xx + self.b[index]) * xx + self.a[index])
...
class IsotopeSplineXMLParser:
    ...
    def parse_document(self, root):
        ...
        for model_ele in root.findall("model"):
            ...
            if isotope >= num_models: continue
            if num_sulfur >= num_models_sulfur or num_sulfur < 0: continue
    ...
    def parse_model(self, model_ele):
        ...
        coefficients = self.decode_double_list(self.get_text_value(model_ele, "coefficients"))
...
class IsotopeSplineDB:
    ...
    def estimate_from_peptide_weight_and_sulfur(self, mono_mass, max_isotope, num_sulfur):
        if self.in_model_bounds(mono_mass, num_sulfur):
            result = IsotopeDistribution(max_isotope)
            for isotope in range(max_isotope + 1):
                result.intensities[isotope] = self.models[num_sulfur][isotope].eval(mono_mass)
            return result
        else:
            return(IsotopeDistribution(max_isotope))
            #raise RuntimeError("Request out of range.")

    def estimate_for_precursor_from_weights_and_sulfur(self, mono_peptide_mass,
                                                       max_isotope, 
                                                       precursor_sulfur,
                                                       iso2efficiency):
        
        precursor_iso_dist = self.estimate_from_peptide_weight_and_sulfur(mono_peptide_mass, 
                                                                    max_isotope, 
                                                                    precursor_sulfur)
        
        precursor_iso_dist.intensities = np.array(precursor_iso_dist.intensities) * iso2efficiency
        
        return precursor_iso_dist
        

    def estimate_for_fragment_from_weights_and_sulfur(self, 
                                                      mono_peptide_mass, 
                                                      mono_fragment_mass, 
                                                      min_isotope, 
                                                      max_isotope, 
        ...
        fragment = self.estimate_from_peptide_weight_and_sulfur(mono_fragment_mass, 
                                                                max_isotope, 
                                                                fragment_sulfur)
        
        comp_fragment = self.estimate_from_peptide_weight_and_sulfur(mono_peptide_mass - mono_fragment_mass, 
                                                                    max_isotope, 
                                                                    precursor_sulfur - fragment_sulfur)
        
        return self.calc_fragment_isotope_distribution(fragment, comp_fragment, min_isotope, max_isotope, iso2efficiency)

    def in_model_bounds(self, mono_mass, num_sulfur):
        return (num_sulfur < self.models.shape[0]) and (mono_mass < 10000)

    def calc_fragment_isotope_distribution(self, fragment, comp_fragment, min_isotope, max_isotope, iso2efficiency):
        result = IsotopeDistribution(max_isotope)

        for i in range(fragment.size()):
            for isotope in range(min_isotope, max_isotope + 1):
                if isotope >= i and (isotope - i) < comp_fragment.size():
                    result.intensities[i] += iso2efficiency[isotope] * comp_fragment.intensities[isotope-i]
