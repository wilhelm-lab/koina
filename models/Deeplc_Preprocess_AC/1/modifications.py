...
class Unimod:
    ...
    def __init__(self, obo_file_url=OBO_FILE_URL,
                 download_folder="./", no_modification_string="-", reverse_lookup_key="name"):
    ...
    def _parse_file_build_dicts(self):
        ...
        unimod_database = [self._parse_unimod_modification_from_substring(term) for term in term_list]
    ...
    def _build_unimod_lookup_dicts(self, unimod_database):
        ...
        for entry in unimod_database:
            ...
            if "xref" in entry.keys():
                self.unimod_db_dict[idx].update(self._parse_modification_metadata(entry["xref"]))

     
        #  reverse lookup using the name or the provided argument
        self.reverse_unimod_db_dict = {v[self.reverse_lookup_key]: k for k, v in self.unimod_db_dict.items()}
...
class ProformaParser:
    ...
    def extract_terminal_mods_and_seq(sequence, terminal_sep=TERMINAL_MODIFICATION_SEP):
        ...
        if len(splitted_seq) == 3:
            ...
            raise ValueError("Failed at extracting terminal modifications, invalid input representation.")
