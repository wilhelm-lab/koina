...
if __name__ == '__main__':
    ...
    inputs.append(grpcclient.InferInput(
        'peptides_in_str:0', [batch_size, 1], "BYTES"))

    peptide_seq_in = np.array([["AAAAAKAK"]
                              for i in range(0, batch_size)], dtype=np.object_)
    ce_in = np.array([[0.25] for i in range(0, batch_size)], dtype=np.float32)
    precursor_charge_in = np.array(
        [[1] for i in range(0, batch_size)], dtype=np.int32)
